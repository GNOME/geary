...
public class ConversationMessage : Gtk.Box, Geary.BaseInterface {
    ...
    private class ContactFlowBoxChild : Gtk.FlowBoxChild {
        ...
        private void update() {
            ...
            var address_parts = new Gtk.Box(Gtk.Orientation.VERTICAL, 0);
    ...
    [GtkChild] private unowned Gtk.Box to_header;
    [GtkChild] private unowned Gtk.Box cc_header;
    [GtkChild] private unowned Gtk.Box bcc_header;
    ...
    private async void fill_header_addresses(Gtk.Box header,
                                             Geary.RFC822.MailboxAddresses? addresses,
                                             GLib.Cancellable? cancellable)
        throws GLib.Error {
        if (addresses != null && addresses.size > 0) {
            ContactList box = header.get_first_child() as ContactList;
            if (box != null) {
    ...
    [GtkCallback]
    private void on_address_box_child_activated(ContactList list,
